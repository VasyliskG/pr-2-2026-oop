...
| Чат (demo) | Standalone chat screen backed by an embedded local HTTP server |
...
Під час запуску застосунку також стартує вбудований локальний chat server для екрана `chat.fxml`.
За замовчуванням він слухає `127.0.0.1` на випадковому вільному порту (`chat.embedded.port=0`).
